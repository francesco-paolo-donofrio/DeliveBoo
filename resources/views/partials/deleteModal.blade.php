<!-- Modal -->
<div class="modal fade mt-5" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content f-d-card-register">
      <div class="modal-header">
        <h1 class="modal-title fs-5 fw-bold text-center text-white" id="exampleModalLabel">Cancella piatto</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      
      <div 
       class="modal-body product-name-modal fs-4 fw-bold text-white">
        
      </div>
      <div class="modal-footer">
        <button type="button" class="btn f-d-button-form-cancel text-white" data-bs-dismiss="modal">Chiudi</button>
        <button type="button" class="btn f-d-button-form-cancel-modal text-white">Cancella</button>
      </div>
    </div>
  </div>
</div>